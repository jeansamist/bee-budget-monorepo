...
export const WalletTransfers: FunctionComponent<{ walletId: number }> = ({ walletId }) => {
  const t = useScopedI18n("app.wallets.detail.transactions.transfers")
  ...
  useEffect(() => {
    ;(async () => {
      const response = await getInternalTransfers({ page: currentPage, perPage, walletId })
      if (response.success) {
  ...
  const handleDelete = useCallback(async () => {
    setDeleting(true)
    setTransfers((prev) => prev.filter((t) => !selectedRows.includes(t)))
    await deleteMassInternalTransfers(selectedRows.map((t) => t.id))
    setDeleting(false)
    setSelectedRows([])
